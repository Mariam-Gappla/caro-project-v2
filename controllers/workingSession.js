  ...
  try {
    ...
    // ابحث عن الجلسة النشطة
    const session = await WorkingSession.findOne({ providerId, endTime: null });

    // إذا لم يجد جلسة، بدلاً من إرجاع 400، ارجع 200 لأن الحالة الحالية هي "مغلق" أصلاً
    if (!session) {
      return res.status(200).send({
        status: true, // غيرناها لـ true
        code: 200,
        message: lang == "en" ? "Session already ended" : "الجلسة منتهية بالفعل"
      });
    }
